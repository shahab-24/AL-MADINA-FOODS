  ...
  useEffect(() => {
    const fetchDonorStats = async () => {
      try {
        ...
        if (user?.email) {
          const response = await axios.get("http://localhost:3000/donor-stats", {
        ...
        const allDonorsResponse = await axios.get("http://localhost:3000/donor-stats/all");
    ...
    <div className="bg-gradient-to-br from-purple-200 to-blue-200 p-10 rounded-lg shadow-lg">
      {loading ? (
        ...
        <>
          ...
          {donorStats ? (
            <>
              <p className="text-lg mb-6">You have donated {donorStats.totalDonations} items!</p>
              <p className="text-lg font-semibold">Current Badge: {getBadge(donorStats.totalDonations)}</p>
            </>
          ) : (
            ...
            <table className="table w-full bg-white shadow-lg rounded-lg">
              ...
              <tbody>
                {allDonors.map((donor, index) => (
                  <tr
                    key={donor._id}
                    className={donor._id === user?.email ? "bg-green-100 font-bold" : ""}
                  >
                    <td>{index + 1}</td>
                    <td>{donor._id}</td>
